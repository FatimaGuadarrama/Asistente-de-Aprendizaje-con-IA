...
const QuizResultPage = () => {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      try {
        ...
        toast.error('No se pudieron obtener los resultados del Quiz.');
        console.error(error);
      } finally {
        setLoading(false);
      }
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center">
          <p className="text-slate-600 text-lg"> No se encontraron los resultados del Quiz.</p>
        </div>
      </div>
    );
  }
  ...
  const getScoreMessage = (score) => {
    ...
    if (score >= 80) return '¡Buen trabajo! ';
    if (score >= 70) return '¡Bien Hecho! ';
    if (score >= 60) return '!No estuvo mal! ';
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="mb-6">
        ...
        >
          <ArrowLeft className="w-4 h-4 group-hover:translate-x-1 transition duration-200" strokeWidth={2} />
          Volver al documento
        </Link>
      </div>

      <PageHeader title={`Resultados del quiz: ${quiz.title || 'Sin título'}`} />
      ...
      <div className="space-y-6">
        <div className ="flex items-center gap-3 mb-2">
          ...
          <h3 className="text-lg font-semibold text-slate-900">Revisión detallada</h3>
        </div>
