...
const QuizTakePage = ({documentId}) => {
  ...
  const handleSubmitQuiz = async () => {
    ...
    try {
      ...
      toast.success('¡El cuestionario se envió correctamente!');
      navigate(`/quizzes/${quizId}/results`);
    } catch (error) {
      toast.error(error.message || 'No se pudo enviar el Quiz');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center">
          <p className="text-slate-600 text-lg">No se encontró el Quiz o no contiene preguntas..</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <PageHeader title={quiz.title || 'Tomar Quiz'} />
      
      {/* Progress Bar */}
      <div className="mb-6">
        <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-semibold text-slate-700">
              Pregunta {currentQuestionIndex + 1} of {quiz.questions.length}
            </span>
            <span className="text-sm font-medium text-slate-500">
              {answeredCount} respondida
            </span>
        </div>
