...
const FlashcardManager = ({ documentId, readOnly = false }) => {
  ...
  const handleToggleStar = async (cardId) => {
    try {
      ...
      toast.success("¡Se actualizó el estado de las Flashcards!");
    } catch (error) {
      toast.error("No se pudo actualizar el estado de las Flashcards");
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast.success("Las Flashcards se eliminaron correctamente.");
      ...
      return (
        <div className="flex flex-col items-center justify-center py-16 px-16">
          ...
          <p className="text-sm text-slate-500 mb-8 text-center max-w-sm">
            Genera Flashcards a partir de tus documentos para empezar a aprender 
        ...
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold text-slate-900">
              Tus conjuntos de Flashcards
            </h3>
            <p className="text-sm text-slate-500 mt-1">
              {flashcardSets.length}{" "}
              {flashcardSets.length === 1 ? "set": "sets"} disponibles
            </p>
          </div>
          <button
            onClick={handleGenerateFlashcards}
          ...
          >
            {generating ? (
              ...
              <>
                ...
                Generar nuevo conjunto de Flashcards
              </>
            )}
          </button>
        </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">
                    Creado {moment(set.createdAt).format("MMM D, YYYY")}
                  </p>
                </div>
  ...
  return (
    ...
    <div className="bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-3xl shadow-xl shadow-slate-200/50 p-8">
      ...
      <Modal
        ...
        title="¿Eliminar conjunto de Flashcards?"
      >
        <div className="space-y-6">
          <p className="text-sm text-slate-600">
            ¿Seguro que quieres eliminar estas Flashcards? Esta acción no se 
            puede deshacer y todas las tarjetas se eliminarán permanentemente.
          </p>
